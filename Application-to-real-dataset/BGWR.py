...
def r_phi_new(phi,lat):
    if(lat>20):
        phi = np.array(phi)
        phi_n = scipy.stats.multivariate_normal(phi,pro_later1[task_id]).rvs(1)
        return phi_n
    else:
        phi = np.array(phi)
        phi_n = scipy.stats.multivariate_normal(phi,pro_later2[task_id]).rvs(1)
        return phi_n
#proposal density function for phi
def d_phi_new(phi_n,phi,lat):
    if(lat>20):
        return np.log(scipy.stats.multivariate_normal(phi,pro_later1[task_id]).pdf(phi_n))
    else:
        return np.log(scipy.stats.multivariate_normal(phi,pro_later2[task_id]).pdf(phi_n))
...
for m in range(num_location):
    trace = np.zeros(shape=[re['phi'].shape[0],re['phi'].shape[2]+re['theta'].shape[2]]) 
    for n in range(re['phi'].shape[0]):
        trace[n][0:re['phi'].shape[2]] = re['phi'][n][m]
        trace[n][re['phi'].shape[2]:] = re['theta'][n][m]
    np.savetxt('trace'+str(m)+'rep'+str(rep_id)+'.csv',trace,delimiter=',')

#print(est_phi)
#np.savetxt("est_phi"+str(rep_id)+".csv", est_phi, delimiter=",")
#np.savetxt("upper_phi"+str(rep_id)+".csv", upper_phi, delimiter=",")
#np.savetxt("median_phi"+str(rep_id)+".csv", median_phi, delimiter=",")
#np.savetxt("lower_phi"+str(rep_id)+".csv", lower_phi, delimiter=",")
#print(est_theta)
#np.savetxt("est_theta"+str(rep_id)+".csv", est_theta, delimiter=",")
#print([h,re['ELPD']])
