...
for i in range(5):
    for j in range(5):
        x=i+1
        y=j+1
        phi=[1,0.5*np.sqrt((x-5.5)**2+(y-5.5)**2),0.1*(np.sin(x)+np.cos(y)+4)]
        theta = float(np.random.uniform(0,0.1,1))
        for n in range(100):
            covariate=[1,float(np.random.uniform(0,1,1)),float(np.random.uniform(2,7,1))]
